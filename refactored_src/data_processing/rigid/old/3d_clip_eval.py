...
各行には label -> clip_index のマッピングを付与します。

出力:
  data/eval/clip_emb/{clip_index}.pt  (ユニークラベル数分)
  data/eval/eval.csv (emb_index列を追加/更新)
"""

save_dir = "data/eval/clip_emb"
...
# 既存の clip_index があってユニークラベル数と .pt が一致していれば再計算をスキップするロジックを入れても良いが、
...
for start in tqdm(range(0, len(unique_labels), batch_size), desc="embed unique labels"):
    ...
    for label, vec in zip(batch_labels, feats_cpu):
        clip_index = len(embeddings)
        embeddings.append(vec)
        label_to_index[label] = clip_index

# 保存 (index順に {i}.pt)
for i, vec in enumerate(embeddings):
    torch.save(vec, os.path.join(save_dir, f"{i}.pt"))

# DataFrame に clip_index 列を付与
train_df["clip_index"] = train_df["label"].map(label_to_index).astype(int)
...
pd.DataFrame({"label": unique_labels, "clip_index": [label_to_index[l] for l in unique_labels]}).to_csv(mapping_path, index=False)
