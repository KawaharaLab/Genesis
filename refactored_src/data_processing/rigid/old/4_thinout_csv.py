...
N = 15  # "hold" and "no-contact" should be N% of the whole dataset
P = N / 100.0
...
print(f"stable (raw)	{len(stable)}")
print(f"no contact (raw)	{len(nc)}")
print(f"drop (raw)	{len(drop_data)}")

O = data_len - len(stable) - len(nc) - len(drop_data)
print(f"others (O)	{O}")


# The theorical number of target k to keep
k_theoretical = (P * O) / (1 - 2 * P) if O > 0 else min(len(stable), len(nc))
k = int(np.floor(k_theoretical))

# adjust if larger than class size
max_possible = min(len(stable), len(nc))
if k > max_possible:
...
keep_stable = choose_keep(stable, k)
keep_nc = choose_keep(nc, k)

drop_stable = set(stable) - keep_stable
drop_nc = set(nc) - keep_nc

to_drop = sorted(drop_stable.union(drop_nc).union(set(drop_data)))

df_final = df.drop(index=to_drop)
final_size = len(df_final)

final_stable = len(keep_stable)
final_nc = len(keep_nc)
prop_stable = final_stable / final_size if final_size else 0
prop_nc = final_nc / final_size if final_size else 0

print(f"drop stable: {len(drop_stable)} (from {len(stable)})")
print(f"drop nc:   {len(drop_nc)} (from {len(nc)})")
print(f"total drop: {len(to_drop)}")
print(f"final size: {final_size} (was {data_len})")
print(f"final stable kept: {final_stable} (was {len(stable)}) {N}%")
