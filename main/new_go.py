...
from make_step import final_make_step 
...
STEP_DF = pd.DataFrame(columns=['action','step','hand_coordinate','object bounding box'])
step_no = 0
...
def setup_paths(name, target_choice):
    ...
    return {
        ...
        "steps": os.path.join(csv_base, f'{name}_{MATERIAL_TYPE}_steps_{target_choice}.csv'),
...
def get_bounding_box(gso_object):
    ...
    particle_positions_np = gso_object.get_state().pos.detach().cpu().numpy()
    min_x = np.min(particle_positions_np[0], axis=0)[0]
    max_x = np.max(particle_positions_np[0], axis=0)[0]
    min_y = np.min(particle_positions_np[0], axis=0)[1]
    max_y = np.max(particle_positions_np[0], axis=0)[1]
    min_z = np.min(particle_positions_np[0], axis=0)[2]
    max_z = np.max(particle_positions_np[0], axis=0)[2]
...
def run_rotation(scene, cam, franka, gso_object, df, deform_csv, paths, target_choice):
    ...
    # Update position of robot and object
    STEP_DF.loc[len(STEP_DF)] = ['start', step_no, end_effector.get_pos(), get_bounding_box(gso_object)]
    ...
    if lower_obj_bound[2] > 0.01:
        pick = 'picked up'
    else:
        print("######################### Completing motion... #########################")
        
        final_make_step(scene, cam, franka, df, paths['photo'], PHOTO_INTERVAL, gso_object, deform_csv, name)
        pick = 'not picked up'
        ...
        # Move CSV file (with safety check)
        if os.path.exists(paths['csv']):
            shutil.move(paths['csv'], not_picked_up_csv_dir)
        else:
            print(f"Warning: source CSV path {paths['csv']} does not exist.")

        # Move deformation CSV file (with safety check)
        if os.path.exists(paths['deform_csv']):
            shutil.move(paths['deform_csv'], not_picked_up_csv_dir)
        else:
            print(f"Warning: source deformation CSV path {paths['deform_csv']} does not exist.")
        ...
        deform_csv.to_csv(os.path.join(not_picked_up_csv_dir, f'{name}_{MATERIAL_TYPE}_deform_{target_choice}.csv'), index=False)
        # print(f"Saved deform data -> {paths['deform_csv']}")
        STEP_DF.to_csv(os.path.join(not_picked_up_csv_dir, f'{name}_{MATERIAL_TYPE}_steps_{target_choice}.csv'), index=False)
        # Move steps CSV file (with safety check)
        exit()


    STEP_DF.loc[len(STEP_DF)] = [pick, step_no, end_effector.get_pos(), get_bounding_box(gso_object)]
    ...
    for action in actions:
        STEP_DF.loc[len(STEP_DF)] = [f'rotation {rotate_num}', step_no, end_effector.get_pos(), get_bounding_box(gso_object)]
...
def main(obj_path, target_choice='soft'):
    ...
    os.makedirs(paths['photo'], exist_ok=True)
