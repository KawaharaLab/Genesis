    ...
    if t % photo_interval == 0:
        for camera_angle in range(3):
            if camera_angle == 0:
                ...
                cam.set_pose(pos = (2.1, -1.2, 0.1), lookat = (0.45, 0.45, 0.5))
                # cam.set_pose(pos = (2.1, -1.2, 0.1), lookat = franka.get_link("hand").get_pos())
            elif camera_angle == 1:
                # cam.set_pose(pos = (-1.5, 1.5, 0.25), lookat = (0.45, 0.45, 0.4))
                cam.set_pose(pos = (-1.5, 1.5, 0.25), lookat = (0.45, 0.45, 0.4))
                # cam.set_pose(pos = (-1.5, 1.5, 0.25), lookat = franka.get_link("hand").get_pos())
            elif camera_angle == 2:
                # cam.set_pose(pos = (2, 2, 0.1), lookat = (0, 0, 0.1))
                cam.set_pose(pos = (2, 2, 0.1), lookat = (0, 0, 0.1))
                # cam.set_pose(pos = (2, 2, 0.1), lookat = franka.get_link("hand").get_pos())
...
def final_make_step(scene, cam, franka, df, photo_path, photo_interval, gso_object, deform_csv, name, gripper_force=0.0):
    """フランカを目標位置に移動させるステップ関数"""
    scene.step()
    t = int(scene.t) - 1
