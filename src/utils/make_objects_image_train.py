...
def main(target_choice: str = 'none'):
    ...
    valid_objects = valid_objects[:42]
    ...
    # 1. グリッドサイズを計算
    nx = int(np.ceil(np.sqrt(num_objects * 2 / 3)))
    ...
    z_offset = horizontal_dist * 0.8

    cam_x = original_lookat[0]
    cam_y = original_lookat[1] - 3 * half_range_y
    ...
    # 5. カメラをセット
    cam = scene.add_camera(res=(1280, 720), pos=new_cam_pos, lookat=new_lookat, fov=30)
    ...
    filepath = "camera_test_train.png"
