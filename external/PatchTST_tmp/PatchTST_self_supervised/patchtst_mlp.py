...
class MLPContrastive(nn.Module):
    """Per-variable temporal MLP encoder for force time-series contrastive learning."""
    ...
    def __init__(
        ...
        self.encoder = nn.Sequential(*[ResidualMLPBlock(hidden_dim, dropout) for _ in range(depth)])

        if hidden_dim == out_embed_dim:
            self.proj = nn.Linear(hidden_dim, out_embed_dim)
        else:
            self.proj = nn.Sequential(
                nn.Linear(hidden_dim, hidden_dim),
                nn.GELU(),
                nn.Dropout(head_dropout),
                nn.Linear(hidden_dim, out_embed_dim),
            )
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.transpose(1, 2).contiguous().view(b * v, t)  # [B*V, T]
        z = self.stem(x)
        z = self.encoder(z)
        z = z.view(b, v, -1).mean(dim=1)  # [B, hidden_dim]
        emb = self.proj(z)
...
def choose_mlp_width(
    ...
    candidates = [64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, 448, 512]
...
def parse_args():
    ...
    # MLP
    ap.add_argument("--mlp_hidden_dim", type=int, default=0, help="<=0: auto-match PatchTST scale")
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Path to MLP SSL pretraining checkpoint. stem/encoder weights will be loaded.",
    )
    ...
    ap.add_argument("--save_dir", type=str, default="saved_models/mlp_bert/")
...
def load_mlp_encoder_weights(model: MLPContrastive, pretrained_path: str, device: torch.device):
    ...
    print(f"[load_mlp_encoder_weights] loaded={loaded} skipped={skipped} from {pretrained_path}")
...
def main():
    ...
    wandb.init(project="mlp_bert", config=vars(args))
    ...
    if args.mlp_hidden_dim > 0:
        mlp_width = args.mlp_hidden_dim
        tmp_model = MLPContrastive(
            nvars=nvars,
        ...
        mlp_params = count_trainable_params(tmp_model)
    else:
        mlp_width, mlp_params = choose_mlp_width(
            nvars=nvars,
            seq_len=args.context_points,
            out_embed_dim=args.out_embed_dim,
            depth=args.mlp_depth,
    ...
    print(f"MLP params: {mlp_params:,} (hidden_dim={mlp_width}, depth={args.mlp_depth})")
    ...
    if (not args.from_scratch) and args.pretrained_path:
        load_mlp_encoder_weights(model, args.pretrained_path, device)
    else:
        print("[info] Training from scratch (no MLP pretrained encoder loaded).")
