...
class ForceContrastiveDataset(Dataset):
    """
    Returns (force_seq, text_embedding)
    force_seq: [seq_len, nvars]
    text_embedding: tensor [dim]
    """
    def __init__(self, data_dir: str, index_csv: str,
        ...
        for _, row in df.iterrows():
            ...
            tmp = "text_emb" if mode == "train" else "text_emb/label"
            emb_file = os.path.join(data_dir, tmp, f"{row[self.emb_index]}.pt")
            ...
            if not torch.isfinite(t_emb).all():
                ...
                if debug_prints < debug_limit:
                    print(f"[ForceContrastiveDataset] non-finite text emb emb_index={row[self.emb_index]} file={emb_file} total={nf_total}")
                    debug_prints += 1
                continue  # drop this pair

            # Valid pair -> append
            self.force_segments.append(seg)
            self.text_embeddings.append(t_emb.float())
    ...
    def __getitem__(self, idx):
        ...
        y = self.text_embeddings[idx]                          # [dim]
        return x, y
...
def parse_args():
    ...
    ap.add_argument('--save_dir', type=str, default='saved_models/contrastive/')
...
def main():
    args = parse_args()
    set_seed(args.seed)
    train_data_dir  = args.data_dir + "train_old/"
    eval_data_dir   = args.data_dir + "eval_heavy/"
    ...
    dataset = ForceContrastiveDataset(train_data_dir, args.index_csv, args.context_points, cols, mode="train")
    ...
    # Validation dataset
    try:
        val_dataset = ForceContrastiveDataset(eval_data_dir, args.val_index_csv, args.context_points, cols, mode="eval")
    ...
    if args.from_scratch:
        project_name = 'patchtst_scratch'
    else:
        project_name = 'patchtst_ssl_based'
