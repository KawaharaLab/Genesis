__all__ = ['TrackTimerCB', 'TrackTrainingCB', 'PrintResultsCB', 'TerminateOnNaNCB',
            'TrackerCB', 'SaveModelCB', 'EarlyStoppingCB', 'WandbLoggingCB']
...
class WandbLoggingCB(Callback):
    def __init__(self, project="icra_ssl", config=None):
        super().__init__()
    ...
    def before_fit(self):
        if self.run_finder: # lr_finder 中は初期化しない
            self._enabled = False
            return
        try:
            import wandb
            api_key = "c85b817c62f441243d232b381088358e72fa2b19"

            if not wandb.run:
                wandb.login(key=api_key, relogin=True)
                self.run = wandb.init(project=self.project, config=self.config, reinit=True, mode='online')
            else:
                self.run = wandb.run
        except Exception as e:
            print(f'[WandbLoggingCB] disabled: {e}')
            self._enabled = False
    ...
    def after_epoch(self):
        ...
        idx = -1
        log_data = {k: v[idx] for k,v in rec.items() if len(v)>0 and k != 'epoch'}
        log_data['epoch'] = rec['epoch'][idx]
        try:
            import wandb
            wandb.log(log_data)
        except Exception as e:
            print(f'[WandbLoggingCB] log error: {e}')
...
class ValidateFiniteCB(Callback):
    """Validate that inputs and losses are finite; raise RuntimeError immediately if not.

    - Checks xb/yb on both train/valid before forward.
    - Checks loss (and optionally predictions) after each batch on both train/valid.
    """
    def __init__(self, check_pred: bool = False):
        super().__init__()
        self.check_pred = check_pred

    def _assert_finite(self, name: str, t: torch.Tensor):
        if t is None:
        ...
        shape = tuple(t.shape)
        total = t.numel()
        nonfinite_total = (~finite_mask).sum().item()
        # Per-sample counts if batched
        per_sample = None
        if t.dim() >= 1:
            per_sample = (~finite_mask.view(t.size(0), -1)).sum(dim=1).tolist()
        # Per-channel counts if channel-last
        per_channel = None
        if t.dim() >= 2:
            try:
                per_channel = (~finite_mask).sum(dim=tuple(i for i in range(t.dim()-1))).tolist()
            except Exception:
                per_channel = None
        # Finite stats
        ...
        msg = (
            ...
            f"epoch={getattr(L,'epoch',None)} iter={getattr(L,'iter',None)} "
            f"shape={shape} nonfinite={nonfinite_total}/{total}."
        )
        if stats is not None:
            msg += f" finite_stats={stats}"
        if per_sample is not None:
            # show first few
            head = per_sample[:8]
            msg += f" per_sample_nonfinite(head)={head}"
        if per_channel is not None and isinstance(per_channel, list):
            head_ch = per_channel[: min(16, len(per_channel))]
            msg += f" per_channel_nonfinite(head)={head_ch}"
    ...
    def before_batch_train(self):
        try:
            ...
            return
        self._assert_finite('xb(train)', xb)
        if yb is not None:
            self._assert_finite('yb(train)', yb)

    def after_batch_train(self):
        if self.check_pred:
            self._assert_finite('pred(train)', self.pred)
        self._assert_finite('loss(train)', self.loss)
    ...
    def before_batch_valid(self):
        try:
            ...
            return
        self._assert_finite('xb(valid)', xb)
        if yb is not None:
            self._assert_finite('yb(valid)', yb)

    def after_batch_valid(self):
        if self.check_pred:
            self._assert_finite('pred(valid)', self.pred)
        self._assert_finite('loss(valid)', self.loss)
...
class SaveModelCB(TrackerCB):
    ...
    def after_fit(self):
        ...
        if not self.every_epoch and self.global_rank == self.save_process_id:
            if getattr(self, 'last_saved_path', None):
                self.learner.load(self.last_saved_path, with_opt=self.with_opt)
            else:
                print('[warn] No checkpoint saved; skip restore in SaveModelCB.after_fit')
