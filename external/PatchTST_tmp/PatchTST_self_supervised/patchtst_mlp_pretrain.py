...
class MLPPatchAutoEncoder(nn.Module):
    """Patch-wise MLP autoencoder. Input/Output: [B, num_patch, nvars, patch_len]."""

    def __init__(self, patch_len: int, hidden_dim: int, depth: int, dropout: float):
        super().__init__()
        # Keep stem/encoder names for downstream weight loading in patchtst_mlp.py.
    ...
    def forward(self, xb_patch_masked: torch.Tensor):
        ...
        x = xb_patch_masked.reshape(b * l * v, p)
        z = self.stem(x)
        z = self.encoder(z)
        out = self.decoder(z)
        return out.view(b, l, v, p)
...
def parse_args():
    ...
    # MLP-specific architecture knobs
    parser.add_argument("--mlp_hidden_dim", type=int, default=0, help="<=0: auto-match PatchTST param scale")
    ...
    parser.add_argument("--save_dir", type=str, default="saved_models/force/masked_mlp/based_model/")
...
def main():
    ...
    wandb.init(project="icra_ssl_mlp", config=vars(args))
    ...
    if args.mlp_hidden_dim > 0:
        mlp_w = args.mlp_hidden_dim
        mlp_p = count_trainable_params(
            MLPPatchAutoEncoder(
                patch_len=args.patch_len,
                hidden_dim=mlp_w,
    ...
    print(f"MLP pretrain params: {mlp_p:,} (hidden_dim={mlp_w}, depth={args.mlp_depth})")
    ...
    best_val = float("inf")
    best_path = os.path.join(args.save_dir, f"mlp_pretrained_model_id{args.model_id}_{run_name}.pth")
