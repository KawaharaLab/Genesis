...
class KinematicEntity(Entity):
    ...
    @gs.assert_built
    def get_root_COM(self, envs_idx=None):
        """Return the center of mass of the kinematic tree containing the base link."""
...
class RigidEntity(KinematicEntity):
    ...
    def get_links_contact_force(self, links_idx_local=None, envs_idx=None, sensor=True, *, unsafe=False):
        """Return net direct-contact forces for selected links.

        When ``sensor`` is true, vectors are expressed in each link's local frame;
        otherwise they are expressed in the world frame.
        """
        links_idx = self._get_global_idx(links_idx_local, self.n_links, self._link_start, unsafe=unsafe)
        tensor = qd_to_torch(self._solver.dyn_state.links.contact_force, envs_idx, links_idx, transpose=True, copy=True)
        if sensor:
            quat = qd_to_torch(self._solver.dyn_state.links.quat, envs_idx, links_idx, transpose=True, copy=True)
            tensor = gu.inv_transform_by_quat(tensor, quat)
        return tensor[0] if self._solver.n_envs == 0 else tensor

    def get_links_contact_torque(self, links_idx_local=None, envs_idx=None, sensor=True, *, unsafe=False):
        """Return net direct-contact torques about the selected link origins.
        ...
        links_idx = self._get_global_idx(links_idx_local, self.n_links, self._link_start, unsafe=unsafe)
        tensor = qd_to_torch(
            self._solver.dyn_state.links.contact_torque, envs_idx, links_idx, transpose=True, copy=True
        )
        if sensor:
            quat = qd_to_torch(self._solver.dyn_state.links.quat, envs_idx, links_idx, transpose=True, copy=True)
            tensor = gu.inv_transform_by_quat(tensor, quat)
