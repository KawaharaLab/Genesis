...
def func_update_contact_force(
    ...
    for i_l, i_b in qd.ndrange(n_links, _B):
        ...
        dyn_state.links.contact_torque[i_l, i_b] = qd.Vector.zero(gs.qd_float, 3)
    ...
    for i_b in range(_B):
        ...
        for i_c in range(collider_state.n_contacts[i_b]):
            ...
            force_a = -force
            force_b = force
            torque_a = (contact_pos - dyn_state.links.pos[contact_data_link_a, i_b]).cross(force_a)
            torque_b = (contact_pos - dyn_state.links.pos[contact_data_link_b, i_b]).cross(force_b)
            dyn_state.links.contact_torque[contact_data_link_a, i_b] = (
                dyn_state.links.contact_torque[contact_data_link_a, i_b] + torque_a
            )
            dyn_state.links.contact_torque[contact_data_link_b, i_b] = (
                dyn_state.links.contact_torque[contact_data_link_b, i_b] + torque_b
            )
