...
class ConstraintSolverIsland:
    ...
    def _func_update_contact_force(self, i_island: int, i_b: int):
        ...
        for i_island_col in range(self.contact_island.island_col.n[i_island, i_b]):
            ...
            pos_a = self._solver.links_state.pos[link_a, i_b]
            pos_b = self._solver.links_state.pos[link_b, i_b]
            torque_a = (self._collider._collider_state.contact_data.pos[i_col, i_b] - pos_a).cross(-force)
            torque_b = (self._collider._collider_state.contact_data.pos[i_col, i_b] - pos_b).cross(force)
            self._solver.links_state.contact_torque[link_a, i_b] = (
                self._solver.links_state.contact_torque[link_a, i_b] + torque_a
            )
            self._solver.links_state.contact_torque[link_b, i_b] = (
                self._solver.links_state.contact_torque[link_b, i_b] + torque_b
            )
